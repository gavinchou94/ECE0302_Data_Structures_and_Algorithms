...
TEST_CASE("Test list properties after removals", "[ArrayList]")
{
   ...
   REQUIRE(listPtr->remove(0) == false);  // 0 is invalid index to remove
   REQUIRE(listPtr->remove(-1) == false); // -1 is invalid index to remove
   displayList(listPtr);
   delete listPtr;
}

TEST_CASE("Test list replace", "[ArrayList]")
{
   ListInterface<std::string> *listPtr = new ArrayList<std::string>();
   std::string data[] = {"one", "two", "three", "four", "five"};
...
TEST_CASE("Test copy assignment operator of LinkedList", "[LinkedList]")
{
   ...
   // write your statements here to test copy assignment operator below
