...
// What is the alg efficiency of this method?
// Hint: which two parts does it depend on?
...
// private
// alg efficiency O(n) here, n is the size of the bag due to the sequential search
...
// If some preprocessing work is done before using getIndexOf(), e.g., Balanced BST
// the alg efficiency of getIndexOf() can be reduced to O(log n).

// When we analyze the rest of methods using this new implementation of getIndexOf(),
// we assume that the preprocessing work is done at no cost and all public methods keep the same