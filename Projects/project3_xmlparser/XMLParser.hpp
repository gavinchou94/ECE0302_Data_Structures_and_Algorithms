...
public:
	...
	/** No custom destructor or copy constructor needed, no dynamic memory here. */

	/** Scans and tokenizes the input string into XML markup and content. Returns true if all tokens
			in the input string are valid. Valid markup (tokens) are the characters included in an '<' '>' character
	...
	/** Parses a valid tokenized string (stored internally after a successful call to tokenizeInputString)
			...
			in the Project Description, where opening tags are replaced with a start tag,
			and the closing tag is replaced by the corresponding end tag. For tag matching, the start tag
	...
	/** Empties and then fills a given vector with the tokenized input string. This
			method returns a vector whether or not the tokenized input string was valid XML.
			The vector elements are of type TokenStruct. This data type has two fields,
			"tokenString" and "tokenType". The type for "tokenType" is an enum
	...
	/** Determines if an element name is contained in a valid XML input string.
	...
	/** Returns the number of times an element name is contained in a valid XML input string.
